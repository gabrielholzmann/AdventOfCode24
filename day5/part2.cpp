...
void sortPage(const manual &m, std::vector<int> &page) {
    auto pageSorter = [&](int &a, int &b) {
        auto pageOrdering = m.ordering.find(a);

        if (pageOrdering != m.ordering.end()) {
            auto &mustComeAfter = pageOrdering->second;

            if (std::find(mustComeAfter.begin(), mustComeAfter.end(), b) != mustComeAfter.end()) {
                return true;
            }
        }
        return false;
    ...
    std::sort(page.begin(), page.end(), pageSorter);
}

int scanManual(manual &m) {
    int total = 0;
